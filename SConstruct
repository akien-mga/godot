...
if env_base["use_ninja"]:
    ...
    if env_base._get_major_minor_revision(scons_raw_version) >= (4, 2, 0):
        env_base.SetOption("experimental", ["ninja"])
        env_base["NINJA_DISABLE_AUTO_RUN"] = True
        env_base.Tool("scons_ninja", toolpath=["misc/scons"])
        ...
        raise SCons.Errors.BuildError(errstr="ninja generation requires SCons >= 4.2")
